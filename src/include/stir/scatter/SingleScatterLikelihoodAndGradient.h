...
public:
    ...
    shared_ptr<ProjData> low_res_jacobian(const ProjData& data,const ProjData &add_sino,std::vector<VoxelsOnCartesianGrid<float> > &gradient_image_array,const bool compute_gradient = true ,const bool isgradient_mu = true,const float rescale = 1.F);
    void low_res_ratio(const ProjData& data,const ProjData &add_sino, ProjData &v_est, std::vector<float> &ratio);
    shared_ptr<ProjData> likelihood_and_gradient_scatter(const ProjData &data, const ProjData &add_sino, VoxelsOnCartesianGrid<float>& gradient_image,const bool compute_gradient, const bool isgradient_mu);
    private:
    ...
    void low_res_jacobian_for_view_segment_number(const ProjData&data,const ProjData&add_sino,std::vector<VoxelsOnCartesianGrid<float> > &gradient_image_array,const ViewSegmentNumbers& vs_num, const float rescale, const bool compute_gradient,const bool isgradient_mu);
